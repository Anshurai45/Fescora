...
export async function sendContactEnquiry(enquiry) {
  ...
  const emailClient = transporter()
  await emailClient.sendMail({ from: env.fromEmail, to: env.contactReceiverEmail, replyTo: enquiry.email, subject: `New Website Enquiry - ${enquiry.service || 'General enquiry'} - ${enquiry.name}`, html: `<div style="font-family:Arial,sans-serif;max-width:650px;margin:auto"><h2 style="color:#DD2E18">New Website Enquiry</h2><p>A visitor has submitted a contact enquiry through the Fescora Management website.</p><table style="width:100%;border-collapse:collapse;background:#fff7e4">${rows}</table></div>` })
}
